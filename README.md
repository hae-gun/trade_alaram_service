...
종목 검색은 KIS 현재가 API를 직접 호출하지 않습니다. 서버가 KIS 거래종목코드 마스터 파일을 시작 시점과 평일 오전 7시 30분에 동기화하고, 검색 API는 저장된 `stocks` 테이블만 조회합니다.
기본 소스는 KOSPI/KOSDAQ 거래종목코드 마스터이며, 보통주뿐 아니라 `삼성전자우(005935)` 같은 우선주 거래종목코드도 포함합니다.
